...
def light_embedding_2d_rot_invariant_multi(I, imax, L, w, N, div, isRandomThresh):
    ...
    params = [(np.pi * (i * 360.0 / div) / 180, m, I, imax, L,
               w, N, anglemask) for i in range(np.int32(div))]
...
def light_embedding_main(Iv, Nv, L, w, rotdiv, validind, isRandomThresh):
    ...
    imax = imax[valid]
    if rotdiv > 1:
        embed, mask, nm, rot, rows, cols = light_embedding_2d_rot_invariant_multi(
            Iv, [imax], L, w, Nv, rotdiv, isRandomThresh)
    else:
        embed, mask, nm, rot, rows, cols = light_embedding_2d_rot_invariant(
            Iv, [imax], L, w, Nv, rotdiv, isRandomThresh)
...
def prep_data_2d_from_images_cycles(dirlist, dirname, scale, w, rotdiv_in, rotdiv_on):
    ...
    for d in dirlist:
        ...
        nml = nml[:, :, ::-1]
        nml = 2 * nml - 1
        nml = cv2.resize(nml, None, fx=scale, fy=scale,
        ...
        inboundary = cv2.resize(
            inboundary, None, fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
        inboundary = np.where(inboundary > 0)
        inboundary_ind = inboundary[0] * height + inboundary[1]
        onboundary = cv2.imread(onboundary_path, -1)
        onboundary = cv2.resize(
            onboundary, None, fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
        ...
        for i in range(numLight):
            ...
            cv2_im = cv2.resize(cv2_im, (height, width),
                                interpolation=cv2.INTER_NEAREST)
            I[i, :, :] = cv2_im
        ...
        embed_in, mask_in, nm_in = light_embedding_main(
            Iv, Nv, L, w, rotdiv_in, inboundary_ind, True)
        embed_on, mask_on, nm_on = light_embedding_main(
            Iv, Nv, L, w, rotdiv_on, onboundary_ind, True)
...
def prep_data_2d_from_images_test(dirlist, scale, w, rotdiv, index=-1):
    ...
    for d in dirlist:
        ...
        if index == -1:
            # if dirpath ends in '/' basename returns the empty string
            setName = os.path.basename(dirpath.rstrip('/'))
            if setName == 'bearPNG':
        ...
        for i, idx in enumerate(index):
            ...
            cv2_im = cv2.imread(image_path, -1) / 65535.0
            cv2_im = (cv2_im[:, :, 0] / Li[i, 0] + cv2_im[:, :,
                      1] / Li[i, 1] + cv2_im[:, :, 2] / Li[i, 2]) / 3
            cv2_im = cv2.resize(cv2_im, None, fx=scale,
...
def TestNetwork(model, Sv, Nv, Rv, IDv, Szv, showFig, isTensorFlow):
    ...
    for i in range(numData):
        ...
        for r in range(rotdiv):
            embed_div = S[:, r, :, :]
            if isTensorFlow:
                embed_div = np.reshape(
                    embed_div, (embed_div.shape[0], embed_div.shape[1], embed_div.shape[2], 1))
            else:
                embed_div = np.reshape(
                    embed_div, (embed_div.shape[0], 1, embed_div.shape[1], embed_div.shape[2]))
            # predict
            ...
            for k in range(len(ID)):
                ...
                for l in range(3):
                    Err[ID[k], l] = 180 * \
                        math.acos(
                            min(1, abs(n.dot(np.transpose(nt))))) / math.pi
                error = error + 180 * \
                    math.acos(min(1, abs(n.dot(np.transpose(nt))))) / math.pi
            print('%d ' % i + '[Angle %d] Ave.Error = %.2f ' %
                  (r, (error / len(ID))))
        ...
        cv2.imwrite("err.png", Err)
        cv2.imwrite("normat_est.png", (Nest[..., ::-1] * 0.5 + 0.5) * 255)
